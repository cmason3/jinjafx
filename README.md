...
This list of dicts will contain all the row and column data that JinjaFx is currently traversing through. The first row will contain the header row (a list) with subsequent rows containing the row data (a dict using column index as key) - it is accessed using `jinjafx.data[row][col]`. If you wish to access the columns via their case-sensitive name then you can also use `jinjafx.data[row]['FIELD']` on rows > 0.
